...
PRINTIT="true" # /bin/true accepts any option but never prints anything
...
if dmidecode="@@dmidecode@@/bin/dmidecode" && [ -r /dev/mem -a -x "$dmidecode" ]
then
...
@@kmod@@/bin/modprobe battery 2> /dev/null || true
